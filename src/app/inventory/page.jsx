...
export default function Inventory() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setTableRows((prevRows) =>
            prevRows.filter((product) => product.id !== idProducto)
          );
        } else {
  ...
  const filteredRows = useMemo(() => {
    return tableRows.filter(
      (row) =>
        (row.descripcion ? row.descripcion.toLowerCase() : "").includes(
          searchTerm.toLowerCase()
        ) ||
        (row.marca ? row.marca.toLowerCase() : "").includes(
          searchTerm.toLowerCase()
        ) ||
        (row.referencia ? row.referencia.toLowerCase() : "").includes(
          searchTerm.toLowerCase()
        ) ||
        (row.modelo ? row.modelo.toLowerCase() : "").includes(
          searchTerm.toLowerCase()
        )
    );
